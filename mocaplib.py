...
#Persistance of cluster numbers from frame to frame
import math
import numpy as np
...
def findMin(vector):
    #find the index of the smallest element in an 
    # array
    smallest = vector[i]
    index = 0
    for i in range(len(vector)):
...
def findStretch(inertiaTensor):

    qrp = np.linalg.qr(inertiaTensor)
    rotc = qrp[0]
    spread = qrp[1]
    sprtot = np.linalg.det(spread)
...
def cluster(points, thresh):
    ...
    npts = (len(points)-2)/3
    for j in range(0,npts):
        x = float(points[0 + 3*j])
        y = float(points[1 + 3*j])
        z = float(points[2 + 3*j])
        ndata[0].append(x)
        ndata[1].append(y)
        ndata[2].append(z)
...
def parseFrame(rawIn):
    ...
    for record in records:
        ...
        # a new record is delineated by the double [[
